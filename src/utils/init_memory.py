...
async def initialize_creator_memory(group_id: int):
    ...
    existing_memories = db_manager.get_user_specific_memories(group_id, creator_name, limit=1)
    ...
    try:
        ...
        db_manager.update_user_impression(group_id, creator_name, impression)
    ...
    try:
        ...
        for memory in user_memories:
            db_manager.add_user_specific_memory(group_id, creator_name, memory)
    ...
    # 6. 添加知识三元组
    try:
        db_manager.add_knowledge_triplet(
            group_id, 
            creator_name, 
            "是", 
            f"{bot_config.bot_name}的创造者", 
            1.0
        )
        db_manager.add_knowledge_triplet(
            group_id, 
    ...
    try:
        db_manager.update_user_relationship(group_id, creator_name, delta_favorability=20, new_status="死党")
    ...
    if bot_config.creator_id:
        try:
            db_manager.update_user_id_map(group_id, creator_name, bot_config.creator_id)
