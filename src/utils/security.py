...
class InputValidator:
    ...
    def validate_text(text: str, max_length: int = 5000) -> Dict[str, Any]:
        ...
        if len(text) > max_length:
            ...
            return {"valid": False, "sanitized": text[:max_length], "reason": f"文本长度超过限制 ({max_length} 字符)"}

        # 检查危险模式
        for pattern in InputValidator.DANGEROUS_PATTERNS:
            if re.search(pattern, text, re.IGNORECASE):
                logger.warning(f"检测到危险模式: {pattern}")
                return {"valid": False, "sanitized": "", "reason": "包含非法字符或命令"}
        ...
        return {"valid": True, "sanitized": text, "reason": None}
    ...
    def validate_json(json_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "data": None, "reason": f"JSON 格式错误: {str(e)}"}
