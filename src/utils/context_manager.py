...
class ContextManager:
    ...
    def truncate_messages(
        self, messages: List[Dict], model_alias: str, max_output_tokens: int = 500, reserve_ratio: float = 0.1
    ) -> Tuple[List[Dict], int]:
        """
        截断消息列表以适应模型的上下文限制
        ...
        if system_tokens <= available_tokens:
            ...
            # system 消息本身就超限了，保留最后（最重要）的 system 消息
    ...
    def truncate_text(
        self, text: str, model_alias: str, max_output_tokens: int = 500, reserve_ratio: float = 0.1
    ) -> Tuple[str, int]:
        """
        截断文本以适应模型的上下文限制
    ...
    def optimize_context_for_chat(
        ...
        if available_tokens <= 0:
            return {"system_prompt": "", "history": [], "user_context": [], "total_tokens": 0, "truncated": True}
