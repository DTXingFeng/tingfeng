...
async def dream_and_optimize(group_id: int):
    ...
    triplets = db_manager.get_knowledge_triplets(group_id, limit=100)
    patterns = db_manager.get_style_patterns(group_id, limit=50)
    ...
    pattern_str = "\n".join([f"- 情境: {p['context']} | 风格: {p['style_desc']} (权重: {p['weight']})" for p in patterns])
