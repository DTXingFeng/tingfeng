...
class KnowledgeQueryTool(BaseTool):
    ...
    async def execute(self, group_id: int, subject: str = None, limit: int = 10) -> Dict[str, Any]:
        ...
        Returns:
            dict: 查询结果
        """
        triplets = db_manager.get_knowledge_triplets(group_id, subject=subject, limit=limit)
...
class GetCreatorKnowledgeTool(BaseTool):
    ...
    async def execute(self, group_id: int) -> Dict[str, Any]:
        ...
        # 查询以创造者为主体的知识
        triplets = db_manager.get_knowledge_triplets(group_id, subject=bot_config.creator_name, limit=20)
        
        # 查询以创造者为客体的知识
        all_triplets = db_manager.get_knowledge_triplets(group_id, limit=50)
        ...
        return {
            "creator": bot_config.creator_name,
            "knowledge": formatted,
            "count": len(formatted)
        }
...
class AddKnowledgeTool(BaseTool):
    ...
    async def execute(self, subject: str, predicate: str, obj: str, group_id: int, confidence: float = 1.0) -> Dict[str, Any]:
        """
        执行添加知识
        ...
        Returns:
            dict: 添加结果
        """
        db_manager.add_knowledge_triplet(group_id, subject, predicate, obj, confidence)
