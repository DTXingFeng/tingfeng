...
class GetRecentMessagesTool(BaseTool):
    ...
    async def execute(self, group_id: int, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            # 从数据库获取最近的聊天记录
            chat_logs = db_manager.get_chat_log(group_id, limit=limit)
...
class GetMessageContextTool(BaseTool):
    ...
    parameters = {
        ...
        "before": {
            "type": "integer",
            "description": "获取前面的消息数量，默认 5",
            "required": False
        },
        "after": {
            "type": "integer",
            "description": "获取后面的消息数量，默认 0",
            "required": False
        }
    }
    ...
    async def execute(self, group_id: int, before: int = 5, after: int = 0) -> Dict[str, Any]:
        ...
        try:
            # 获取前面的消息
            total = before + after
            chat_logs = db_manager.get_chat_log(group_id, limit=total)
