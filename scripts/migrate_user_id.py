...
def backup_database(db_path: str) -> str:
    ...
    backup_path = (
        db_path_obj.parent / f"{db_path_obj.stem}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}{db_path_obj.suffix}"
    )
...
def migrate_table(conn, table_name: str, has_primary_key: bool = True):
    ...
    if has_primary_key:
        ...
        try:
            ...
            new_sql = original_sql.replace(f"CREATE TABLE {table_name}", f"CREATE TABLE {new_table_name}")

            # 替换主键定义
            if "PRIMARY KEY (group_id, user_name)" in new_sql:
                new_sql = new_sql.replace("PRIMARY KEY (group_id, user_name)", "PRIMARY KEY (group_id, user_id)")
    ...
    try:
        ...
        if duplicates:
            ...
            for group_id, user_id, count in duplicates:
                ...
                print(
                    f"  合并 user_id={user_id}, group_id={group_id}: {count}条记录 -> 好感度={max_favorability}, 状态={latest_status}"
                )

                # 删除所有旧记录
                cursor.execute("DELETE FROM user_relationships WHERE group_id = ? AND user_id = ?", (group_id, user_id))
